...
| **513** | `BEI/ClosedGraphs.lean:452` | `groebner_implies_closed` | **Active work package**: [`guides/work_packages/GROEBNER_IMPLIES_CLOSED_REFACTOR.md`](guides/work_packages/GROEBNER_IMPLIES_CLOSED_REFACTOR.md). Investigated 2026-05-02 by a read-only subagent — `[CARVE-CANDIDATE]`. Four nearly-identical branches (Conditions 1 and 2, each split by `lt_or_gt_of_ne`) share the same `hp_mem` / `hp_eq` / `hdeg` / `lex_lt` / `contra` skeleton up to index permutation. Three new helpers proposed (`cubic_degree`, `cubic_witness`, `extract_ab_from_le`). Estimated 513 → 280–320 LOC, medium risk. |
