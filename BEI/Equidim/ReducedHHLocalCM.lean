...
/-!
# Equidimensional surrogate: reduced HH ring is CM local at its augmentation

This file isolates Session A' of the F2-route global Cohen-Macaulay proof:
the "kill last pair" bridge from the HH ring to the reduced HH ring, the
localisation-quotient commutation, the image of `augIdeal` under the
bridge, the augIdealQuot iso to `Localization.AtPrime augIdealReduced`,
and the assembly `isCohenMacaulayLocalRing_at_augIdealReduced_step`.
...
theorem isCohenMacaulayLocalRing_at_augIdealReduced_base {K : Type*} [Field K]
    (G' : SimpleGraph (Fin 1)) :
...
-/
theorem isCohenMacaulayLocalRing_at_augIdealReduced_step
...
for any HH graph `G` on `Fin (r + 1)`. -/
theorem isCohenMacaulayLocalRing_at_augIdealReduced
