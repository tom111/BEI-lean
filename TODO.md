...
| **274** | `BEI/GroebnerBasis.lean:229` | `groebnerElement_reduced_same_endpoints` | **Active work package**: [`guides/work_packages/GROEBNER_REDUCED_SAME_ENDPOINTS_REFACTOR.md`](guides/work_packages/GROEBNER_REDUCED_SAME_ENDPOINTS_REFACTOR.md). Investigated 2026-05-02 by a read-only subagent — `[CARVE-CANDIDATE]` low risk. The proof reinvents helpers that already exist in `BEI/CoveredWalks.lean` (`mem_internalVertices_of_ne`, `internal_ne_getLast`, `head_of_head?`, `getLast_of_getLast?`) — ~85 LOC of inline duplication. Plus the same-file sister `groebnerElement_leadingMonomial_squarefree` (line 615) repeats ~27 LOC of "endpoint is not internal" reasoning, fixable by a single new helper. Estimated 274 → ~190 LOC, low risk. |
