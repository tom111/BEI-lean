...
/-- The **bipartite edge monomial ideal** associated to `G`: generated by `x_i · y_j`
    where `{i, j+1} ∈ E(G)`. This is the image of the monomial initial ideal under
    the y-predecessor shift `φ`, and corresponds to the edge ideal of the bipartite
    graph `Γ` from the proof of Proposition 1.6.

    Its generators are in bijection with the edges of Γ. The Herzog–Hibi conditions
    (see `HerzogHibiConditions`) characterize when this ideal is Cohen–Macaulay. -/
def bipartiteEdgeMonomialIdeal {n : ℕ} (G : SimpleGraph (Fin n)) :
  ...
  Ideal.span { m | ∃ (i j : Fin n) (hj : j.val + 1 < n),
    G.Adj i ⟨j.val + 1, by omega⟩ ∧ m = X (Sum.inl i) * X (Sum.inr j) }
...
2. **Variable shift** (`yPredVar`, `rename_yPredVar_generator`): The automorphism
   `φ: yⱼ ↦ y_{j-1}` maps these monomials to `{xᵢ y_{j-1}}`, giving the
   bipartite edge ideal of the graph Γ (`bipartiteEdgeMonomialIdeal`).
...
theorem prop_1_6 (n : ℕ) (G : SimpleGraph (Fin n))
  ...
  -- Step 2: Variable shift (per-generator transport is rename_yPredVar_generator)
  -- Step 3: HH conditions on the bipartite graph Γ
  have _hHH := prop_1_6_herzogHibi n G hConn hClosed hCond
  -- Remaining: HH CM theorem + initial ideal CM transfer
  sorry
