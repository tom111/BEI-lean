...
/-!
# Section 3 dimension formula (Corollary 3.3 core)

Core dimension-formula material from Section 3:

* `ringKrullDim_quot_primeComponent` — `dim(R/P_S) = |V| - |S| + c(S)`;
* `corollary_3_3` — `dim(K[x,y]/J_G) = max_{S ⊆ V} (|V| - |S| + c(S))`;
* `corollary_3_3_lower_bound` — `dim(K[x,y]/J_G) ≥ |V| + c(G)`.

The third-isomorphism dimension lemma `ringKrullDim_quotQuot_eq` lives here
because it is a generic dimension utility used by every equidimensional
surrogate downstream of this file.

The equidimensional surrogates of Corollaries 3.4 and 3.7 and the path-graph
and Proposition 1.6 surrogate machinery are split into the sibling files
`BEI/PrimeDecompositionDimension.lean` and `BEI/Prop1_6Equidim.lean`.
...
theorem ringKrullDim_quotQuot_eq
