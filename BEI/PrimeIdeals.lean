...
open Classical in
private lemma primeComponentMap_X_inl (G : SimpleGraph V) (S : Finset V) (i : V) :
    primeComponentMap (K := K) G S (X (Sum.inl i)) =
      if i ∈ S then 0 else X (Sum.inl i : BinomialEdgeVars V) := by
  simp [primeComponentMap, MvPolynomial.aeval_X]

open Classical in
private lemma primeComponentMap_X_inr (G : SimpleGraph V) (S : Finset V) (i : V) :
    primeComponentMap (K := K) G S (X (Sum.inr i)) =
      if i ∈ S then 0
      else X (Sum.inl i : BinomialEdgeVars V) *
        X (Sum.inr (compRep G S i) : BinomialEdgeVars V) := by
  simp [primeComponentMap, MvPolynomial.aeval_X]

open Classical in
private lemma primeComponentMap_x_of_not_mem (G : SimpleGraph V) (S : Finset V)
    {i : V} (hi : i ∉ S) :
    primeComponentMap (K := K) G S (x i) = x i := by
  rw [x, primeComponentMap_X_inl, if_neg hi]

open Classical in
private lemma primeComponentMap_y_of_not_mem (G : SimpleGraph V) (S : Finset V)
    {i : V} (hi : i ∉ S) :
    primeComponentMap (K := K) G S (y i) =
      x i * y (compRep G S i) := by
  simpa [x, y] using (primeComponentMap_X_inr (K := K) G S i).trans (if_neg hi)

open Classical in
private lemma primeComponentMap_minor_sameComponent (G : SimpleGraph V) (S : Finset V)
    {j k : V} (hsc : SameComponent G S j k) :
    primeComponentMap (K := K) G S (x j * y k - x k * y j) = 0 := by
  rw [map_sub, map_mul, map_mul]
  rw [primeComponentMap_x_of_not_mem G S hsc.1]
  rw [primeComponentMap_y_of_not_mem G S hsc.2.1]
  rw [primeComponentMap_x_of_not_mem G S hsc.2.1]
  rw [primeComponentMap_y_of_not_mem G S hsc.1]
  rw [compRep_eq_of_sameComponent G S hsc]
  ring
...
private lemma primeComponent_le_ker (G : SimpleGraph V) (S : Finset V) :
  ...
  · -- x_i for i ∈ S
    ...
    rw [primeComponentMap_X_inl, if_pos hiS]
  · -- y_i for i ∈ S
    change (primeComponentMap G S) (X (Sum.inr i)) = 0
    rw [primeComponentMap_X_inr, if_pos hiS]
  · -- x_j * y_k - x_k * y_j for j,k in same component
    exact primeComponentMap_minor_sameComponent G S hsc
    ...
    · -- d₀ contributes 0; remove it
      ...
      · intro e
        have hfib : (T.erase d₀).filter (fun d => normExp G S d = e) =
          ...
          simpa using Finset.filter_erase (fun d => normExp G S d = e) d₀ T
        rw [hfib]
        ...
        · intro e
          ...
          · -- e = normExp G S d₀
            ...
            have hfib_eq : (T.erase d₀).filter (fun d => normExp G S d = normExp G S d₀) =
              ...
              simpa using
                Finset.filter_erase (fun d => normExp G S d = normExp G S d₀) d₀ T
          ...
          · -- e ≠ normExp G S d₀: d₁ and d₀ not in (T.erase d₀).filter(normExp = e)
            ...
            have hfib_eq' : (T.erase d₀).filter (fun d => normExp G S d = e) =
              ...
              have hd₀_not : d₀ ∉ T.filter (fun d => normExp G S d = e) := by
                intro hd₀e
                exact he (Finset.mem_filter.mp hd₀e).2.symm
              rw [Finset.filter_erase, Finset.erase_eq_of_notMem hd₀_not]
            rw [hfib_eq']
...
theorem X_inl_not_mem_primeComponent [Finite V] (G : SimpleGraph V)
  ...
  change primeComponentMap (K := K) G S (X (Sum.inl v)) = 0 at hker
  rw [primeComponentMap_X_inl, if_neg hv] at hker
...
theorem X_inr_not_mem_primeComponent [Finite V] (G : SimpleGraph V)
  ...
  have hker : (primeComponentMap (K := K) G S).toRingHom (X (Sum.inr v)) = 0 :=
    RingHom.mem_ker.mp (hle h)
  change primeComponentMap (K := K) G S (X (Sum.inr v)) = 0 at hker
  rw [primeComponentMap_X_inr, if_neg hv] at hker
