...
| `[x]` 11 | 1991 → 1455 | `BEI/GroebnerBasisSPolynomial.lean:143` | `theorem_2_1` (load-bearing public Buchberger proof). Done 2026-05-03 across five stages, archived at [`guides/archive/THEOREM_2_1_REFACTOR.md`](guides/archive/THEOREM_2_1_REFACTOR.md). Stage 1 extracted `cov_inr_or_inl_of_admissible_path` (24× reuse across the eight `isRemainder_fij_of_mixed_walk` lambda bodies). Stage 2 extracted `degree_monomial_mul_fij` (5× reuse). Stage 3 extracted `case4_remainder` and `case5_remainder` folding each Case's two endpoint-order subcases into one helper called twice via π↔σ swap. Stage 4 extracted cross-cutting `fij_degree_inr_eq_zero` and `fij_degree_inl_eq_zero` (32× reuse). Stage 5 dropped now-redundant local cov helpers and hoisted `(internalVertices π).Nodup` and `Q ≥ D` boilerplate to outer scope. The originally proposed `case2_3_lt_branch` mega-helper was deliberately skipped — its signature would have grown beyond the body savings (negative-value extraction). Final: 1991 → 1455 LOC (−536, 27%), `theorem_2_1` axioms unchanged ([propext, Classical.choice, Quot.sound]), `lake build BEI.AxiomCheck` clean. |
| `[x]` 12 | 837 → 402 | `BEI/CoveredWalks.lean:1338` | `isRemainder_fij_of_mixed_walk` second-pass refactor (done 2026-05-03). After #10 reduced the mixed-walk proof to 837 LOC by sharing sub-walk extractions with the x/y siblings, the four telescope branches (bad-x / bad-y / proc-y / proc-x) and 8 disjunctive-coverage transfers were still textually parallel. Done across five committed stages, archived at [`guides/archive/MIXED_WALK_REMAINDER_REFACTOR.md`](guides/archive/MIXED_WALK_REMAINDER_REFACTOR.md). Stage 1 extracted `exists_min_bad_vertex` / `exists_max_bad_vertex` (3× reuse across all three flagship walk theorems). Stage 2 extracted `x_telescope_monomial_eq` / `y_telescope_monomial_eq` (the algebraic identity reused 6×) and the bad-case packagers `telescope_step_x_bad` / `telescope_step_y_bad` (collapse all four bad-vertex branches in `_covered_walk{,_y}` and `_mixed_walk` to one-liners each). Stage 3 extracted `ne_head?_of_internal` / `ne_getLast?_of_internal` and pulled the basic internal-vertex helpers above the three theorems (12+ replacement sites). Stage 4 extracted four `sub_add_single_*_eval_*` Finsupp evaluators (replacing 20 inline 4-line blocks), reflowed the proc-y/proc-x narrowly-broken code, and pulled `mem_internalVertices_of_ne` up so the no-bad-vertex `mem_internal` and `hint_σ_τ` blocks across all three theorems collapse to ~5 LOC each. The mixed-walk proof itself shrank 837 → 402 LOC (~52%), `BEI/CoveredWalks.lean` shrank 2390 → 1960 LOC (−430, ~18%), `theorem_2_1` axioms unchanged, `lake build BEI.AxiomCheck` clean after every stage commit. |
...
| **513 → ~281** | `BEI/ClosedGraphs.lean:452` | `groebner_implies_closed` | Done 2026-05-02. Stage 0 dropped the unused inline `extract_b`. Stage 1 extracted `cubic_degree` (one private lemma replacing 8 inline 9-line `degree_mul`/`degree_X` blocks). Stage 2 extracted `extract_cond1` / `extract_cond2`, two private Finsupp lemmas replacing the four 30-LOC `(a, b)`-extraction epilogues. Stage 3 (`cubic_witness`) deliberately skipped — parameterisation grew rather than shrank the proof. Axioms unchanged across `groebner_implies_closed`, `theorem_1_1`, `theorem_2_1`, `proposition_1_6`, `corollary_3_4`, `corollary_3_7_cm_fin`, `monomialInitialIdeal_isCohenMacaulay` ([propext, Classical.choice, Quot.sound]); file 978 → 862. Work package archived. |
| **274 → ~210** | `BEI/GroebnerBasis.lean:239` | `groebnerElement_reduced_same_endpoints` | Done 2026-05-02. Stages 1–2 dropped inline reinventions of `mem_internalVertices_of_ne` and `internal_ne_getLast` (already in `BEI/CoveredWalks.lean`; the latter pair was lifted from `private` to package-visible). Stage 3 extracted `endpoints_notMem_internalVertices` (one private lemma) and used it in both the primary and the sister `groebnerElement_leadingMonomial_squarefree`, removing ~33 LOC of parallel duplication at the sister too. Stage 4 (`firstDiffIdx`) deliberately skipped — would have saved ~4 net lines. File `BEI/GroebnerBasis.lean` shrank 766 → 652 LOC. Axioms unchanged across both targets and the five flagship downstream theorems ([propext, Classical.choice, Quot.sound]). Work package archived. |
...
| `[x]` 247+245+243+179+167 → −172 net | `toMathlib/GradedFiniteFree.lean` and `toMathlib/GradedRegularSop.lean` | `linearIndependent_aeval_cons_step`, `linearIndependent_aeval_of_basis_lift`, `finiteFree_over_mvPolynomial_of_homogeneous_regular_sop`, `exists_homogeneous_nonZeroDivisor_quotient_cm_of_dim_pos`, `ringKrullDim_irrelevant_quotient_eq` | Graded* family refactor done 2026-05-03 across five staged commits, archived at [`guides/archive/GRADED_FINITE_FREE_REFACTOR.md`](guides/archive/GRADED_FINITE_FREE_REFACTOR.md). Stage 0 Mathlib hunt found `LinearEquiv.isWeaklyRegular_congr` and `Module.Basis.mapCoeffs` already upstream. Stage 1 extracted `localizationAtIrrelevantOfQuotientSpan_ringEquiv` (the duplicated 70-LOC `heq_ring + hPC + hequiv_bridge` block in two consumers, 77 LOC saved). Stage 2 added `QuotSMulTop.linearEquivQuotSpanSingleton` for the canonical `R`-linear equivalence between the two views of `R ⧸ ⟨r⟩`. Stage 3 pulled `aeval_finCons_eq_polynomial_aeval` and `quotientMk_aeval_finCons_eq_aeval_coeff_zero` out of `linearIndependent_aeval_cons_step`. Stage 4's high-gain swing replaced an 85-LOC hand-built `repr_LE` LinearEquiv in the `finiteFree_over_mvPolynomial_of_homogeneous_regular_sop` zero branch with a one-line `Module.Basis.mapCoeffs` call (76 LOC). Stage 5 trimmed three dead `have` blocks (10 LOC). Net: 172 LOC across both files. AxiomCheck clean after every stage commit; flagship axioms unchanged. |
