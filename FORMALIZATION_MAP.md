...
| Proposition 1.6 | `proposition_1_6`, `groebnerDeformation_cm_transfer`, `monomialInitialIdeal_isCohenMacaulay`, `prop_1_6_equidim` | `BEI/Proposition1_6.lean`, `BEI/GroebnerDeformation.lean`, `BEI/Equidim.lean`, `BEI/PrimeDecompositionDimension.lean` | Sorry | Paper-faithful `proposition_1_6` proved modulo one sub-sorry in `BEI/GroebnerDeformation.lean`: local CM at the irrelevant ideal (`tildeJ_quotient_isCohenMacaulayLocal_at_irrelevant`). `tildeJ_quotient_isCohenMacaulay` is now a one-line application of the graded local-to-global theorem + the local-CM sub-sorry. BEI-side graded plumbing — `defWeight`, `defGrading`, `tildeJQuotGrading`, `tildeJ_isHomogeneous`, `tildeJQuotGrading_connectedGraded`, proper-ness — fully formalised. `tildeJ_polyT_colon_eq` is proved (Gröbner division + standard-form argument). `tildeJ_flat_over_polyT` and the `IsSMulRegular` steps are proved via torsion-free-is-flat over PID `K[t]`. Deformation framework — ring, generators, fiber isos, `baseQuotEquiv` — fully built |
