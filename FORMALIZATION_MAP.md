...
| Proposition 1.6 | `proposition_1_6`, `proposition_1_6_dim_formula`, `pathGraph_binomialEdgeIdeal_isCohenMacaulay`, `pathGraph_binomialEdgeIdeal_ringKrullDim`, `groebnerDeformation_cm_transfer`, `monomialInitialIdeal_isCohenMacaulay`, `prop_1_6_equidim` | `BEI/Proposition1_6.lean`, `BEI/GroebnerDeformation.lean`, `BEI/Equidim.lean`, `BEI/PrimeDecompositionDimension.lean` | Sorry (upstream only) | Paper-faithful `proposition_1_6` fully proved **BEI-side**. The only remaining transitive `sorryAx` comes from the dormant Case-C sorry of `toMathlib/GradedCM.lean` (graded local-to-global CM for non-homogeneous primes — a commutative-algebra gap independent of BEI). BEI-side ingredients: weighted grading (`defWeight`, `isWeightedHomogeneous_fijTilde`), graded quotient structure (`tildeJQuotGrading`), connectedness, `tildeJ_isHomogeneous`, `tildeJ_polyT_colon_eq`, flatness over PID `K[t]`, `tildeJ_t_isSMulRegular`, `specZeroQuotEquiv`, and `tildeJ_quotient_isCohenMacaulayLocal_at_irrelevant` (all clean-axiom). **Add-ons (2026-04-21)**: `proposition_1_6_dim_formula` (dim formula under Prop 1.6 — axiom-clean via `prop_1_6_equidim` + `corollary_3_4_equidim`), path-graph examples `pathGraph_satisfiesProp1_6Condition`, `pathGraph_binomialEdgeIdeal_isCohenMacaulay` (CM for paths; transitive GradedCM sorry), `pathGraph_binomialEdgeIdeal_ringKrullDim` (dim = n+1 for paths; **clean axioms**) |
