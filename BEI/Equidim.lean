  ...
  · -- Case p ⊆ augIdeal: CM by localization transitivity.
    ...
    exact cast (show IsCohenMacaulayLocalRing (Localization.AtPrime q) =
      IsCohenMacaulayLocalRing (Localization.AtPrime p) by
        ...
        rw [hpc]) hCM_q
  · -- Case p ⊄ augIdeal: requires graded algebra (dehomogenization).
    --
    -- The standard argument (Bruns–Herzog 2.1.3, Goto–Watanabe):
    -- Since p ⊄ augIdeal, some degree-1 element r ∈ augIdeal is not in p,
    -- hence is a unit in R_p. Dehomogenize by r: the degree-0 part
    -- R[r⁻¹]₀ is a polynomial ring quotient, and R_p ≅ (R[r⁻¹]₀)_{p₀}[r±¹].
    -- The localization R_{augIdeal} ≅ (R[r⁻¹]₀)_{m₀} is CM (by hypothesis),
    -- so (R[r⁻¹]₀)_{p₀} is CM (by p₀ ⊆ m₀ + localization transitivity),
    -- and the Laurent extension preserves CM.
    --
    -- Formalizing this requires graded-ring dehomogenization infrastructure
    -- not yet available in this repo. The variable-inversion structural
    -- lemmas (exists_var_not_mem, isUnit_algebraMap_mkI_X,
    -- algebraMap_mkI_X_eq_zero_of_edge) capture the first step of this
    -- argument but the dehomogenization isomorphism is not yet formalized.
    sorry
