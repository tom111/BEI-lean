...
| `[~]` 11 | 1848 | `BEI/GroebnerBasisSPolynomial.lean:143` | `theorem_2_1` (load-bearing public Buchberger proof). **Active work package**: [`guides/work_packages/THEOREM_2_1_REFACTOR.md`](guides/work_packages/THEOREM_2_1_REFACTOR.md). Re-investigated 2026-05-03 by a read-only subagent — `[CARVE-CANDIDATE-HIGH-VALUE]`. The 8 leaf bodies inside Cases 2 & 3 (and Cases 4/5) share the same `(fun w hw => ...)` coverage-lambda skeleton fed to `isRemainder_fij_of_mixed_walk` — ~50 LOC × 8 sites of pure label permutation. Plus the post-`h₁ h₂` ring/`degree_ne`/`isRemainder_add` finisher (~25 LOC × 8 sites). Plus the i<k / k<i sister symmetry that opens with `fij_antisymm` reductions to the `<` shape. Four helpers proposed; estimated 1848 → 900–1000 LOC (~−850, **biggest single LOC win still available**), medium risk. The recent Buchberger and `groebner_implies_closed` carves landed axiom-clean and the new `BEI/AxiomCheck.lean` regression check now provides a one-line cross-check (`lake build BEI.AxiomCheck`), so the helper budget for splitting `theorem_2_1` is healthier than at the time of the original deferral. |
